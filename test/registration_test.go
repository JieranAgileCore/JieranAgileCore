...
import (
	...
	// ausf_context "github.com/JieranAgileCore/ausf/context"
	...
	"github.com/JieranAgileCore/openapi/Npcf_PolicyAuthorization"
...
func TestXnHandover(t *testing.T) {
	...
	// send NGSetupRequest Msg
	sendMsg, err = test.GetNGSetupRequest([]byte("\x00\x01\x01"), 24, "JieranAgileCore")
	assert.Nil(t, err)
	_, err = conn.Write(sendMsg)
	assert.Nil(t, err)
